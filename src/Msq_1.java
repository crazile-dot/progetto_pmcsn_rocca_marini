...
class MsqSum_1 {                      /* accumulated sums of                */
  double service;                   /*   service times                    */
  long   served;                    /*   number served                    */
}

class MsqEvent_1 {                     /* the next-event list    */
...
class Msq_1 {
  ...
  public static void main(String[] args) {
    ...
    System.out.println("Inizializzazione: " + event[0].t);
    event[0].x   = 1;
    for (s = 1; s <= SERVERS; s++) {
    ...
    while ((event[0].x != 0) || (number != 0)) {
      ...
      if (e == 0) {                                  /* process an arrival*/
        number++;
        event[0].t        = m.getArrival(r);
        System.out.println("Primo evento: " + event[0].t);
